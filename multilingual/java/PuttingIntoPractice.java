...
public class PuttingIntoPractice {
    public static void main(String[] args) {
        ...
        String list4 = transactions.stream().map(Transaction::getTrader).map(Trader::getName).distinct().sorted()
                .reduce("", (n1, n2) -> n1 + n2);
        ...
        Optional<Integer> list7 = transactions.stream().map(Transaction::getValue).reduce(Integer::max);
        System.out.println(list7);

        Optional<Transaction> list8 = transactions.stream().reduce((transaction1,
                transaction2) -> transaction1.getValue() > transaction2.getValue() ? transaction2 : transaction1);
        System.out.println(list8);

        Optional<Transaction> list81 = transactions.stream().min(Comparator.comparing(Transaction::getValue));
